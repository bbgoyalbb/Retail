from fastapi import Depends, FastAPI, APIRouter, HTTPException, Query, UploadFile, File, Header
...
if not mongo_url:
    raise RuntimeError("MONGO_URL environment variable is required. Copy backend/.env.example to backend/.env and fill in your values.")
_db_name = os.environ.get('DB_NAME')
if not _db_name:
    raise RuntimeError("DB_NAME environment variable is required. Copy backend/.env.example to backend/.env and fill in your values.")
...
# Optional API key guard for destructive endpoints (backup/restore).
# Set ADMIN_API_KEY env var to enable; leave unset to skip auth (dev mode).
...
async def seed_data():
    ...
    try:
        import openpyxl
        wb_path = os.environ.get("SEED_FILE_PATH", "/tmp/retail_book.xlsm")
...
@api_router.get("/backup", dependencies=[Depends(require_admin_key)])
...
@api_router.post("/restore", dependencies=[Depends(require_admin_key)])
